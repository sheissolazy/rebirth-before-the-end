...
import type { GameState, MetaProgress, NewGameOptions, WeekReport } from '../engine/types'
...
export function useStore(): Store {
  ...
  const newGame = useCallback((opts: Omit<NewGameOptions, 'meta'>) => {
    commit(engine.newGame(content, { ...opts, meta }))
    setReport(null)
  }, [meta, commit])
  ...
  return useMemo(() => ({
    state, meta, report, error, newGame, act, endWeek,
    dismissReport: () => setReport(null), finishLife, buyShop,
    clearError: () => setError(null), abandon: () => { commit(null); setReport(null) },
  }), [state, meta, report, error, newGame, act, endWeek, finishLife, buyShop, commit])
}
