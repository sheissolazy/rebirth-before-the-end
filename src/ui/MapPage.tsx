        ...
        <section key={loc.id}>
          ...
          <ul className="mt-1 grid gap-2 sm:grid-cols-2">
            {events.filter((e) => e.locationId === loc.id).map((e) => (
              <li key={e.id}>
                <button onClick={() => setOpen(e)} className={`w-full rounded-lg border p-2 text-left text-sm ${placedIds.has(e.id) ? 'border-amber-500 bg-amber-950/30' : 'border-zinc-800 bg-zinc-900'}`}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="shrink-0 text-xs text-zinc-500">
                      {(e.storylineNpcId || e.once) && <span className="mr-1 rounded bg-sky-900/60 px-1 text-sky-300">{t('event.story')}</span>}
                      ⚡{e.energy ?? 2}
                    </span>
                  </div>
